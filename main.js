...
async function start_animation() {
    ...
    async function animateElementRemoval(startX, startY, endX, endY, centerX, centerY, duration) {
        const rect = draw.rect(40, 20).fill('red').center(startX, startY);
        ...
        rect.animate(duration)
            .during(function(pos, i) { // pos is 0 to 1 progress
            const point = path.pointAt(pathLength * pos); // Get point at current progress
            rect.move(point.x - 10, point.y - 10); // Move circle (subtract half its size)
        }).ease('linear');
    ...
    while (jos.hasNext()) {
        ...
        if (run){
            ...
            animateElementRemoval(currentElement.x, currentElement.y, elementToCrossOf.x, elementToCrossOf.y, elementXOffset, elementYOffset, 500)
            await new Promise(r => setTimeout(r, 500));
